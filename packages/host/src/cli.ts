...
async function runServer(opts: { port: number; dataDir: string }): Promise<void> {
...
async function runSetPassword(opts: { dataDir: string; userId?: string; password?: string }): Promise<void> {
...
main().catch((err) => {
  console.error('[margo-host]', (err as Error).message)
  process.exit(1)
})
