...
const ROLE_RANK = { read: 0, write: 1, admin: 2 };
/**
 ...
 * second lookup.
 */
export async function authenticate(req, cfg) {
    ...
    if (!m)
        throw new AuthError(401, 'expected `Authorization: Bearer <token>`');
 ...
 */
export async function authorize(cfg, user, projectSlug, required) {
    if (user.isSuperuser)
        return;
